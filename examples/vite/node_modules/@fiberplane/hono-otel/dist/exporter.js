...
import { isWrapped } from "./utils/index.js";
 ...
 */
export class FPOTLPExporter {
    constructor(config, fetchFn, logger) {
        this.url = config.url;
        this.headers = Object.assign({}, defaultHeaders, config.headers);
        this.fetchFn = fetchFn;
        this.logger = logger;
        if (isWrapped(this.fetchFn)) {
            this.logger.error("FPOTLPExporter's fetch function is already instrumented, so we cannot send traces without causing an infinite loop");
