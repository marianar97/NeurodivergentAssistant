...
describe("request utils", () => {
    describe("getRedactedUrl", () => {
        it("should not redact any parameters when tracing everything", () => {
            ...
            const url = new URL("https://api.example.com/test?token=secret&name=test");
            const redactedUrl = getRedactedUrl(url, config);
            expect(redactedUrl).toBe("https://api.example.com/test?token=secret&name=test");
        });
        it("should redact sensitive query parameters when not tracing everything", () => {
            ...
            const url = new URL("https://api.example.com/test?token=secret&name=test");
            const redactedUrl = getRedactedUrl(url, config);
            expect(redactedUrl).toBe("https://api.example.com/test?token=REDACTED&name=test");
        });
        it("should handle multiple sensitive parameters", () => {
            ...
            const url = new URL("https://api.example.com/test?token=secret&password=123&key=abc&name=test");
            const redactedUrl = getRedactedUrl(url, config);
            expect(redactedUrl).toBe("https://api.example.com/test?token=REDACTED&password=REDACTED&key=REDACTED&name=test");
        });
