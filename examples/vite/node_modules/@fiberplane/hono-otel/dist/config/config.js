import { getLogLevel, getMode, getOtelEndpoint, getOtelToken, getServiceName, } from "./resolvers.js";
/**
...
export function resolveConfig(userConfig, env) {
...
function mergeConfigs(fallbackConfig, userConfig) {
    ...
    return {
        libraryDebugMode,
        redactedHeaders: mergeRedactedHeaders(fallbackConfig.redactedHeaders, userConfig?.redactedHeaders),
        redactedQueryParams: mergeRedactedQueryParams(fallbackConfig.redactedQueryParams, userConfig?.redactedQueryParams),
        monitor: Object.assign({}, fallbackConfig.monitor, userConfig?.monitor),
    };
}
function mergeRedactedHeaders(fallbackHeaders, userHeaders) {
    const lowerCaseUserHeaders = (userHeaders ?? []).map((header) => header.toLowerCase());
    return [...fallbackHeaders, ...lowerCaseUserHeaders];
}
function mergeRedactedQueryParams(fallbackParams, userParams) {
